...
#♦binariser la variable dépendante


dbfinal1 <- dbfinal1 %>% 
  mutate(dummy = Hmisc::cut2(dbfinal1$victories, c(0,1))) 
levels(dbfinal1$dummy)[levels(dbfinal1$dummy)=="0"] <- "0"
levels(dbfinal1$dummy)[levels(dbfinal1$dummy)=="1"] <- "1"
levels(dbfinal1$dummy)[levels(dbfinal1$dummy)=="[1,6]"] <- "1"
levels(dbfinal1$dummy)

dbfinal1 <-as.tibble(dbfinal1 %>% group_by(player) %>%  mutate(lag_vic = lag(dummy, order_by = year)))
